...
上传 `.txt` 文件时，后台会解析 `名称:"网址"` 格式的条目（每行一条，行尾文字作为介绍）：
...
- **只有 1 个网址** → 首页该条目为蓝色按钮，**点击直接跳转**到该网站。
- **有多个网址** → 首页显示为「名称（N 个链接）」，点击进入**分页列表**，逐条展示名称、网址与介绍；
  列表左上角有「← 返回」按钮回到文件列表。
- 支持中文引号 `“...”` / 英文引号 `"..."`，中文冒号 `：` / 英文冒号 `:`。
- 若只写 `"www.bilibili.com"`（不带 http），会自动补全为 `https://`。
- 重复网址会自动去重；若没有 `名称:"网址"` 结构，则退回取第一个被引号包裹的网址。
